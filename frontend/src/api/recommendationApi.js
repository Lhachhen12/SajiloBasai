import { PROPERTIES, delay } from './api';
...
const searchProperties = (query) => {
  ...
  const locations = [
    'kathmandu', 'patan', 'bhaktapur', 'pokhara', 'lalitpur',
    'boudha', 'kirtipur', 'dhapasi', 'thamel', 'jawalakhel',
    'baneshwor', 'dillibazar', 'gongabu', 'koteshwor', 'chabahil'
  ];
  ...
  const budgetTerms = {
    cheap: [
      'cheap', 'affordable', 'low cost', 'budget', 'economical',
      'low price', 'low priced', 'inexpensive', 'reasonable', 'value'
    ],
    expensive: [
      'expensive', 'luxury', 'premium', 'high end', 'deluxe',
      'high price', 'high priced', 'upscale', 'exclusive', 'posh'
    ]
  };
  ...
  // Apply all filters
  const filteredAndSortedResults = results.filter(property => {
    // Price filters (multiple possible)
    const priceMatch = priceFilters.length === 0 ? true : 
    ...
    const budgetMatch = 
      budgetFilter === 'cheap' ? property.price <= 25000 : // Increased threshold
      budgetFilter === 'expensive' ? property.price >= 20000 :
      true;
    
    // Type matching (can match multiple types)
    const typeMatch = matchedTypes.length === 0 ? true : 
      matchedTypes.includes(property.type);
    
    // Location matching (can match multiple locations)
    const locationMatch = matchedLocations.length === 0 ? true :
...
const recommendProperties = (propertyId, allProperties, searchCriteria = null) => {
  if (searchCriteria) {
  ...
  return allProperties
    .filter(p => {
      ...
      if (currentProperty.location && p.location !== currentProperty.location) return false;
    ...
    .sort((a, b) => {
      ...
      const bedroomDiffA = Math.abs((a.bedrooms || 0) - (currentProperty.bedrooms || 0));
      const bedroomDiffB = Math.abs((b.bedrooms || 0) - (currentProperty.bedrooms || 0));
...
export const getRecommendations = async (propertyId = null, searchCriteria = null) => {
  try {
