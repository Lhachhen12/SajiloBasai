...
export const trackBookingEvent = async (type, bookingData, req) => {
  try {
    const sessionId =
      req.headers['x-session-id'] || `session_${Date.now()}_${Math.random()}`;
    const userAgent = req.get('User-Agent');
    ...
    const eventData = {
      type, // 'booking_start' or 'booking_complete'
      sessionId,
      userId: req.user?.id || null,
      ...
      location,
      metadata: {
        conversionValue: bookingData.amount || 0,
    ...
    console.error('Booking event tracking error:', error);
  }
};
