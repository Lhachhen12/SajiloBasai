...
const Sidebar = ({ isOpen, toggleSidebar, isDark, navigate }) => {
  ...
  const navItems = [
    ...
    { path: '/add-listings', name: 'Add Listings', icon: <FiPlusCircle className="w-5 h-5" /> },
    ...
    <>
      ...
      >
        ...
        <div className={`flex items-center justify-between h-16 px-4 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
          <div className={`transition-all duration-300 ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 invisible'}`}>
                  ...
                  >
                    ...
                    <span className="flex items-center relative z-10">
                      ...
                      <span className={`
                        ...
                        ${isOpen ? 'opacity-100 visible translate-x-0' : 'opacity-0 invisible -translate-x-4 w-0'}
                        ${isMobile ? 'opacity-100 visible w-auto translate-x-0' : ''}
                      `}>
                        {item.name}
                      </span>
                    </span>
