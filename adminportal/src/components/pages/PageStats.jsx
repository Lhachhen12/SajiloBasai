...
const PageStats = ({ pages, isDark }) => {
  const stats = [
    ...
    {
      title: 'Legal Pages',
      value: pages.filter(page => ['terms', 'privacy'].includes(page.type)).length,
      icon: FiLock,
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {stats.map((stat, index) => {
        ...
        return (
          <div
            key={index}
            className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border rounded-xl p-6 transition-all duration-300 hover:shadow-lg hover:scale-[1.02]`}
          >
